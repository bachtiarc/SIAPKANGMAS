            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        @php
                            $baseJenis = array_filter([
                                'q' => request('q'),
                                'status' => request('status'),
                            ]);
                        @endphp

                        <a href="{{ route('masyarakat.history.index', $baseJenis) }}"
                           class="px-4 py-2 rounded-lg text-sm font-medium transition {{ !request('jenis') ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            Semua
                        </a>

                        <a href="{{ route('masyarakat.history.index', array_merge($baseJenis, ['jenis' => 'submission'])) }}"
                           class="px-4 py-2 rounded-lg text-sm font-medium transition {{ request('jenis') === 'submission' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            Permohonan Informasi
                        </a>

                        <a href="{{ route('masyarakat.history.index', array_merge($baseJenis, ['jenis' => 'consultation'])) }}"
                           class="px-4 py-2 rounded-lg text-sm font-medium transition {{ request('jenis') === 'consultation' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            Konsultasi
                        </a>

                        <a href="{{ route('masyarakat.history.index', array_merge($baseJenis, ['jenis' => 'complaint'])) }}"
                           class="px-4 py-2 rounded-lg text-sm font-medium transition {{ request('jenis') === 'complaint' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            Pengaduan
                        </a>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        @php
                            $baseStatus = array_filter([
                                'q' => request('q'),
                                'jenis' => request('jenis'),
                            ]);
                        @endphp

                        <a href="{{ route('masyarakat.history.index', $baseStatus) }}"
                        ...
                        <a href="{{ route('masyarakat.history.index', array_merge($baseStatus, ['status' => 'pending'])) }}"
                        ...
                        <a href="{{ route('masyarakat.history.index', array_merge($baseStatus, ['status' => 'diproses'])) }}"
                        ...
                        <a href="{{ route('masyarakat.history.index', array_merge($baseStatus, ['status' => 'selesai'])) }}"
                        ...
                        <a href="{{ route('masyarakat.history.index', array_merge($baseStatus, ['status' => 'ditolak'])) }}"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($histories as $row)
                            @php
                                ...
                                if ($row['type'] === 'consultation') $typeBadge = 'bg-purple-100 text-purple-800';
                                elseif ($row['type'] === 'complaint') $typeBadge = 'bg-orange-100 text-orange-800';
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-gray-900">{{ $row['ticket_id'] }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ Str::limit($row['title'], 55) }}</div>
                                </td>

                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $row['date']->format('d M Y') }}</div>
                                    <div class="text-xs text-gray-500">{{ $row['date']->format('H:i') }} WIB</div>
                                </td>
