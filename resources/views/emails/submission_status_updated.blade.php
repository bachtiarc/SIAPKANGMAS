...
@php
    ...
    $badgeClass = match($submission->status) {
        ...
        'completed', 'selesai', 'approved' => 'badge-green',
...
<div class="container">
    ...
    <div class="content">
        <p>Yth. <strong>{{ $submission->user->name ?? 'Pengguna' }}</strong>,</p>

        <p>
            Ada pembaruan pada status tiket pengajuan Anda
            <strong>#{{ $submission->ticket_id }}</strong>.
        </p>
        ...
        <div class="info-row">
            ...
            <div class="info-value">
                {{ optional($submission->updated_at)->format('d F Y, H:i') ?? date('d F Y, H:i') }} WIB
            </div>
        </div>
