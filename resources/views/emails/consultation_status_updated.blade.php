<!DOCTYPE html>
<html>
<head>
    <title>Update Status Konsultasi</title>
</head>
<body style="font-family: Arial, sans-serif; padding: 20px;">
    <h2>Halo, {{ $consultation->user->name }}</h2>
    <p>Ada pembaruan pada tiket konsultasi Anda <strong>#{{ $consultation->ticket_id }}</strong>.</p>
    
    <p>Status Terbaru: <strong>{{ ucfirst($consultation->status) }}</strong></p>
    
    @if($note)
    <div style="background-color: #f3f4f6; padding: 15px; border-left: 4px solid #3b82f6; margin: 20px 0;">
        <p style="margin: 0; font-weight: bold;">Tanggapan / Catatan Admin:</p>
        <p style="margin-top: 5px;">{{ $note }}</p>
    </div>
    @endif

    <p>Silakan login ke dashboard SIAPKANGMAS untuk melihat detail lebih lanjut.</p>
    <br>
    <p>Terima kasih,<br>Tim Admin SIAPKANGMAS</p>
</body>
</html>