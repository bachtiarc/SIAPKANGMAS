...
class ComplaintsExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize
{
    ...
    {
        $q = Complaint::with(['user','category'])->latest();
        ...
        // pengaduan kamu punya dua versi status: pending/diproses/selesai/ditolak atau pending/on_progress/completed/rejected
    ...
    {
        return [
            $item->ticket_number ?? $item->ticket_id ?? $item->id,
